...
| MDX | XHTML |
|---|---|
| `# 标题` (仅 index.mdx 用) | `<h1>标题</h1>` |
| `## N.M 标题` | `<h2>N.M 标题</h2>` |
| `### N.M.K 标题` | `<h3>N.M.K 标题</h3>` |
| `#### 标题` | `<h4>标题</h4>` |
| `##### 标题` | `<h5>标题</h5>` |
| `###### 标题` | `<h6>标题</h6>` |

**注意**：常规章节（非 index.mdx）的正文章节从 `##` 开始，对应 `<h2>`。不要在章节开头加 `<h1>`——章节标题由 OPF/spine 控制，正文内容从 `<h2>` 开始。

例外：`index.mdx`（前言/全景图章）可以使用 `<h1>` 作为顶级标题。
