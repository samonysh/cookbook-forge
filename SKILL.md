...
- `scripts/mdx2tex.mjs` — MDX → ElegantBook LaTeX 转换器（先按 code fence 分块，避免代码内 `#`/`{}`/`_` 被转义；从 frontmatter 读 title；支持 callout→tcolorbox、表格→tabularx、图片→figure、列表→itemize）。
- `scripts/word-count.mjs` — 字数统计（GFM 表格按"表头+分隔行"块计数，不再虚高）。
- `scripts/build-epub.mjs` — MDX → EPUB 完整构建（tokenize-first 转换；callout→div.class；代码块不被 heading 正则误伤；自动拷贝图片并登记 MIME；CSS 追加 callout/math/table/code 样式）。
...
#### 5b. ElegantBook LaTeX + PDF

调用 `elegantbook-latex` skill，按以下流程：

1. 在 `latex-pdf/latex/` 下初始化 ElegantBook 工程（参考 `project-cookbook-latex` 的目录规范）：
   - `main.tex` 全局配置 + `\input{chapters/...}`
   - `chapters/` 每章一个 `.tex`
   - `figures/` 放图片（从 `mdx/public/figures/` 拷贝）
   - `diagrams/` 放图表源
   - `metadata/` 放 sources / word-count / build-notes
2. **MDX → LaTeX 转换**（写 `scripts/mdx2tex.mjs` 或 pandoc + 后处理），关键映射：
   - `# 标题` → `\chapter{}` / `\section{}` / `\subsection{}`
   - `**粗体**` `*斜体*` → `\textbf{}` `\emph{}`
   - 表格 → `booktabs` + `tabularx`，宽表用 `xltabular`/`longtable`
   - 代码围栏 → `tcolorbox + listings` 环境（见 project-cookbook-latex §10）
   - `<div className="callout xxx">` → `\begin{tcolorbox}[...]` 自定义环境
   - 图片 → `\includegraphics` + `\begin{figure}`
   - `$...$` / `$$...$$` 数学公式原样保留
   - 跨章链接 → `\ref{ch:xx}` / `\autoref`
3. 封面图片必须 1280×1024（见 project-cookbook-latex §6），记录来源。
4. 用 XeLaTeX 两遍编译生成 PDF。
5. 强制质量闸门（来自 project-cookbook-latex §质量闸门）：
   - PDF 不得出现 Markdown 表格源码
   - 表格必须 LaTeX 环境，不得过挤
   - 代码块必须 tcolorbox/listings
   - 封面非空
   - 字数达标
   - LaTeX 可复现编译
...
从 `mdx/` 权威内容生成 EPUB（不是从 PDF/LaTeX 反推）：

1. MDX → XHTML（每章一个 xhtml，去掉 JSX 仅保留 HTML 语义标签）。
2. 在 `epub/build/` 下建立 EPUB3 结构：
   - `mimetype`（第一文件，STORED，内容固定 `application/epub+zip`）
   - `META-INF/container.xml`
   - `OEBPS/content.opf`（清单 + spine + 元数据）
   - `OEBPS/toc.ncx` + `nav.xhtml`（导航）
   - `OEBPS/Text/*.xhtml`
   - `OEBPS/Images/`（拷贝图）
   - `OEBPS/Fonts/`（LXGW WenKai 子集化 WOFF2）
   - `OEBPS/css/stylesheet.css`
3. CSS 使用本 Skill 自带的 `assets/stylesheet.template.css` 作为基础样式，若内容包含公式图片场景则额外引入 `assets/stylesheet.formula-image.css`，要求：
   - 强制白底黑字（防编辑器误触发 dark scheme）
   - 中文字体栈：LXGW WenKai / 霞鹜文楷 / Source Han Serif / SimSun
   - 代码字体：LXGW WenKai Mono
   - 代码块 tcolorbox 风格（边框 + 浅灰背景 + 左侧蓝条）
   - 表格 booktabs 风格（顶/底粗线 + 行间细线）
   - 响应式图片
4. 字体必须**子集化嵌入**（14MB TTF → ~300KB WOFF2），用 `scripts/subset-fonts.mjs`：
   - 优先通过 `fonteditor-core`（`npm install fonteditor-core`）从 xhtml 收集实际使用字符后做 TTF→WOFF2 子集化
   - 若未安装，脚本会提示安装命令，并给出 Python (`fonttools+brotli`) fallback 命令
   - 在 `content.opf` 登记 `media-type="font/woff2"`
5. 若内容含"公式即图片"场景，用 `scripts/optimize-formula-images.mjs` 修复 HTML 结构。
6. 用 `scripts/epub-zip.mjs` 打包（手写 ZIP，mimetype STORED 且为第一；零依赖）。
7. 自检：
...
- 不使用 flex/grid 复杂布局
- 颜色用 `!important` 覆盖阅读器主题
- 禁用 `@media (prefers-color-scheme: dark)` 块（防白底白字）
