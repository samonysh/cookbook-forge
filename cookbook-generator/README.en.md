...
> Domain CookBook / Handbook / Survey generator — from multi-source materials to chapterized MDX, then ElegantBook PDF, Nextra website, and optimized EPUB, all in O'Reilly CookBook and Springer Handbook style.

## What It Is

`cookbook-generator` is an orchestrating Skill that produces a complete, in-depth technical book from a user's natural-language request and assorted source materials (web pages, PDFs, EPUBs, GitHub repos, arxiv papers, project docs, etc.).
...
- ✅ Root-level `SKILL.md` (YAML frontmatter + Markdown instructions)
- ✅ `scripts/` executable scripts (`.mjs` / `.py`)
- ✅ `references/` lazily-loaded reference docs (style guides)
- ✅ `assets/` templates and resources (MDX template, Nextra scaffold, CSS, Python tools)
- ✅ Bilingual `README.md` / `README.en.md`

Zip the directory and upload at the SkillHub "Publish Skill" entry.
